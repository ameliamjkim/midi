...
**`Pill` as a separate component, not a `Button` variant.** They have different jobs - `Pill` is a toggle, `Button` is an action. Shared visual primitives (layout, focus, transitions) live in a `_button-blueprint.scss` partial that both `@extend`, so the APIs stay independent without duplicating styles.
